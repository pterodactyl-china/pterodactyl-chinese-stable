...
const schema = object().shape({
    ...
    payload: string().when('action', {
        is: (v) => v !== 'backup',
        then: string().required('必须提供任务有效负载。'),
        otherwise: string(),
    }),
    continueOnFailure: boolean(),
    timeOffset: number()
        .typeError('时间偏移量必须是 0 到 900 之间的有效数字。')
        .required('必须提供时间偏移值。')
        .min(0, '时间偏移必须至少为 0 秒。')
                    ...
                    <div css={tw`flex`}>
                        ...
                        <div css={tw`flex-1 ml-6`}>
                            <Field
                                ...
                                label={'时间偏移量 (秒)'}
                                description={
                                    '上一个任务执行后在运行此任务之前等待的时间。 如果这是计划中的第一个任务，则不会应用此任务。'
                                }
                            />
                        </div>
                    </div>
                        ...
                        {values.action === 'command' ? (
                            ...
                            <div>
                                ...
                                <FormikFieldWrapper name={'payload'}>
                                    <FormikField as={Select} name={'payload'}>
                                        ...
                                        <option value={'restart'}>重新启动服务器</option>
                                        <option value={'stop'}>安全停止服务器</option>
                                        <option value={'kill'}>强制停止服务器</option>
                                    </FormikField>
                                </FormikFieldWrapper>
                            </div>
                        ) : (
                            <div>
                                ...
                                <FormikFieldWrapper
                                    name={'payload'}
                                    description={
                                        '可选的。 在此备份中包括要排除的文件和文件夹。 默认情况下，将使用 .pteroignore 文件的内容。 如果您已达到备份限制，将轮换最旧的备份。'
                                    }
                                >
                                    <FormikField as={Textarea} name={'payload'} rows={6} />
                                </FormikFieldWrapper>
